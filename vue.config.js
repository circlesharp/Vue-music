...
const { DepGraphPlugin } = require('./track-deps/depGraphPlugin')
...
module.exports = {
  devServer: {
    before (app) {
      ...
      // 照葫芦画瓢：后端代理接口 ~~~ 成功！！
      // 但是，不用这个方法，因为没有 Findex
      app.get('/api/getSingerList', function (req, res) {
        const url = 'https://u.y.qq.com/cgi-bin/musicu.fcg'
        // axios 用法：axios.get(url, {[header,] params})
        axios
          .get(url, {
            headers: {
            ...
            params: req.query // 参数原封不动
          })
          .then((response) => {
            res.json(response.data) // 将结果输出给前端
          })
          .catch((e) => {
            console.log(e)
          })
